...
export async function POST(req: NextRequest) {
  try {
    ...
    const { text, userId } = parseResult.data;

    // Fetch user preferences/timezone if available, otherwise default
...
Interpret effort: 'thoda sa' = 0.5h, 'bahut zyada' = 4h+.
    ...
    const prompt = buildExtractionPrompt(text, todayStr, timezone);
    ...
    console.error("POST /api/ai/extract-text error:", error);
    return NextResponse.json(
      { error: "EXTRACTION_FAILED", message: error.message || "Failed to extract commitment from text" },
      { status: 422 }
    );
  }
}
